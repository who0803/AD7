



(() => {

    proObj = {
        0: {
            '0': {
                img: "images/pro/1.png",
                name: "프로그래밍?파이썬으로 시작!",
                price: 12000,
                star: 5,
                badge: true,
            },
            '1': {
                img: "images/pro/2.png",
                name: "IT취업은 자바! 손쉽게 시작!",
                price: 20000,
                star: 4,
                badge: true,
            },
            '2': {
                img: "images/pro/3.png",
                name: "코딩테스트 준비해?C++로 준비하자!",
                price: 8000,
                star: 3,
                badge: true,
            },
            '3': {
                img: "images/pro/4.png",
                name: "30년 개발경력자의 실력향상강의!",
                price: 10000,
                star: 2,
                badge: true,
            },
            '4': {
                img: "images/pro/5.png",
                name: "트렌드언어 파이썬-Django로 취업하자!",
                price: 20000,
                star: 1,
                badge: false,
            },
            '5': {
                img: "images/pro/10.png",
                name: "비전공자들을 위한 JS클래스!",
                price: 13000,
                star: 4,
                badge: false,
            },
        },
        1: {
            '0': {
                img: "images/pro/6.png",
                name: "취업하고싶어? 스프링 쉽게 배우자!",
                progress: 15, // 15일 남음
                badge: true,
            },
            '1': {
                img: "images/pro/7.png",
                name: "00회사 개발자 출신 임병철의 입문자용 NodeJS강의",
                progress: 25,
                badge: true,
            },
            '2': {
                img: "images/pro/8.png",
                name: "Springboot 로 나만의 블로그 만들자!",
                progress: 5,
                badge: false,
            },
            '3': {
                img: "images/pro/9.png",
                name: "신부자의 c#기초 강의",
                progress: 4,
                badge: false,
            },
            '4': {
                img: "images/pro/10.png",
                name: "비전공자들을 위한 JS클래스!",
                progress: 20,
                badge: false,
            },
            '5': {
                img: "images/pro/5.png",
                name: "트렌드언어 파이썬-Django로 취업하자!",
                progress: 20,
                badge: false,
            },
        }
    }

    craftObj = {
        0: {
            '0': {
                img: "images/crafts/1.png",
                name: "가죽 공예",
                price: 10000,
                star: 5,
                badge: true,
            },
            '1': {
                img: "images/crafts/2.jpg",
                name: "포장 공예",
                price: 6000,
                star: 4,
                badge: true,
            },
            '2': {
                img: "images/crafts/3.jfif",
                name: "리본 공예",
                price: 5000,
                star: 3,
                badge: false,
            },
            '3': {
                img: "images/crafts/4.jpg",
                name: "도자기 공예",
                price: 8000,
                star: 2,
                badge: false,
            },
            '4': {
                img: "images/crafts/5.jfif",
                name: "한지 공예",
                price: 10000,
                star: 1,
                badge: false,
            },
            '5': {
                img: "images/crafts/6.jfif",
                name: "종이 공예",
                price: 7000,
                star: 4,
                badge: false,
            },
        },
        1: {
            '0': {
                img: "images/crafts/6.jfif",
                name: "종이 공예",
                progress: 15, // 15일 남음
                badge: true,
            },
            '1': {
                img: "images/crafts/7.jpg",
                name: "뜨개질 공예",
                progress: 25,
                badge: true,
            },
            '2': {
                img: "images/crafts/8.jpg",
                name: "레진 공예",
                progress: 5,
                badge: true,
            },
            '3': {
                img: "images/crafts/9.jpg",
                name: "인형 공예",
                progress: 4,
                badge: true,
            },
            '4': {
                img: "images/crafts/10.webp",
                name: "식물 공예",
                progress: 20,
                badge: false,
            },
            '5': {
                img: "images/crafts/1.png",
                name: "가죽 공예",
                progress: 20,
                badge: false,
            },
        }
    }

    rareObj = {
        0: {
            '0': {
                img: "images/rare/1.jpg",
                name: "주부 30년차, 영양만점 집밥강의",
                price: 10000,
                star: 5,
                badge: true,
            },
            '1': {
                img: "images/rare/2.jpg",
                name: "컴퓨터 수리전문, HW수리 기본강의!",
                price: 12000,
                star: 4,
                badge: true,
            },
            '2': {
                img: "images/rare/3.jpg",
                name: "버스킹만 22년, 기본발성 잡고가자",
                price: 15000,
                star: 3,
                badge: true,
            },
            '3': {
                img: "images/rare/4.jpg",
                name: "경험으로 알려주는 창업특강",
                price: 5000,
                star: 2,
                badge: true,
            },
            '4': {
                img: "images/rare/5.jpg",
                name: "카페사장의 바리스타 특강",
                price: 7000,
                star: 1,
                badge: false,
            },
            '5': {
                img: "images/rare/6.jpg",
                name: "춤 배우고 싶어요? 기본부터 차근차근! ",
                price: 10000,
                star: 4,
                badge: false,
            },
        },
        1: {
            '0': {
                img: "images/rare/6.jpg",
                name: "춤 배우고 싶어요? 기본부터 차근차근! ",
                progress: 15, // 15일 남음
                badge: true,
            },
            '1': {
                img: "images/rare/7.jpg",
                name: "[사진사 37년]사진을 잘 찍으려면?구도부터 잡자!",
                progress: 25,
                badge: true,
            },
            '2': {
                img: "images/rare/8.jpg",
                name: "토익독학으로 990점달성, 노하우공유! ",
                progress: 5,
                badge: true,
            },
            '3': {
                img: "images/rare/9.jpg",
                name: "과거 대기업 면접관의 자기소개특강",
                progress: 4,
                badge: false,
            },
            '4': {
                img: "images/rare/10.jpg",
                name: "사람의 마음을 읽는법, 심리학특강",
                progress: 20,
                badge: false,
            },
            '5': {
                img: "images/rare/1.jpg",
                name: "주부 30년차, 영양만점 집밥강의",
                progress: 20,
                badge: false,
            },
        }
    }

    const primary = document.querySelector('.primary');

    function createLec(obj, str) {
        const lecContainer = document.createElement('div');
        lecContainer.classList.add('lecContainer');

        const lecListName = document.createElement('div');
        lecListName.classList.add('lecListName');
        lecListName.innerHTML = "# " + str + " " + `<i class="fas fa-chevron-right"></i>`;
        lecListName.style.fontWeight = 'bolder';

        // 리스트 이름 클릭 이벤트
        lecListName.addEventListener('click', function () {
            localStorage.setItem('obj', JSON.stringify(obj));
            localStorage.setItem('titleName', str);
            location.href = 'lectureList.html'; // 이동
        });


        const lecList = document.createElement('div');
        lecList.classList.add('lecList');

        for (key in obj[0]) {
            const lec = document.createElement('div');
            lec.classList.add('lec');
            lec.dataset.key = key;

            const badge = document.createElement('div');
            badge.classList.add('badge');



            const lecImg = document.createElement('div');
            lecImg.classList.add('lecImg');
            lecImg.style.backgroundImage = "url(" + obj[0][key].img + ")";


            const lecDesc = document.createElement('div');
            lecDesc.classList.add('lecDesc');

            const lectName = document.createElement('div');
            lectName.classList.add('lectName');
            lectName.innerHTML = obj[0][key].name;
            lecDesc.appendChild(lectName);

            const lectStar = document.createElement('div');
            lectStar.classList.add('lectStar');
            lecDesc.appendChild(lectStar);
            for (let i = 0; i < obj[0][key].star; i++) {   // 별 숫자만큼
                lectStar.insertAdjacentHTML("beforeend", `<i class="fas fa-star"></i>`);
            }
            for (let i = 0; i < 5 - obj[0][key].star; i++) {   // 별 숫자만큼
                lectStar.insertAdjacentHTML("beforeend", `<i class="far fa-star"></i>`);
            }


            const lecPrice = document.createElement('div');
            lecPrice.classList.add('lecPrice');
            lecPrice.innerHTML = "₩ " + obj[0][key].price;


            if (obj[0][key].badge) { // 뱃지가 트루여야만
                lec.appendChild(badge);
            }
            lecDesc.appendChild(lecPrice);
            lec.appendChild(lecImg);
            lec.appendChild(lecDesc);
            lecList.appendChild(lec);

            lec.addEventListener('click', function (e) {


                //console.log(e.target.parentNode);
                console.log(obj[0]);
                if (e.target.parentNode.classList.contains('lec')) {
                    console.log(e.target.parentNode.dataset.key);
                    localStorage.setItem('key', e.target.parentNode.dataset.key);
                }
                else if (e.target.parentNode.classList.contains('lecDesc')) {
                    console.log(e.target.parentNode.parentNode.dataset.key);
                    localStorage.setItem('key', e.target.parentNode.parentNode.dataset.key);
                }
                else if (e.target.parentNode.classList.contains('lectStar')) {
                    console.log(e.target.parentNode.parentNode.parentNode.dataset.key);
                    localStorage.setItem('key', e.target.parentNode.parentNode.parentNode.dataset.key);
                }
                //console.log(e.currentTarget.dataset.flag);
                localStorage.setItem('obj', JSON.stringify(obj[0]));
                location.href = 'signUp.html';
            });
        }


        lecContainer.appendChild(lecListName);
        lecContainer.appendChild(lecList);
        primary.appendChild(lecContainer);

    }


    createLec(rareObj, "이색 강의!");
    createLec(proObj, "프로그래밍");
    createLec(craftObj, "공예");



    // 슬라이드 이벤트
    const lecList = document.querySelector('.lecList');
    let isMouseDown = false;
    let startX, scrollLeft;

    lecList.addEventListener('mousedown', (e) => {
        isMouseDown = true;
        lecList.classList.add('active');

        startX = e.pageX - lecList.offsetLeft;
        scrollLeft = lecList.scrollLeft;
    });

    lecList.addEventListener('mouseleave', () => {
        isMouseDown = false;
        lecList.classList.remove('active');
    });

    lecList.addEventListener('mouseup', () => {
        isMouseDown = false;
        lecList.classList.remove('active');
    });

    lecList.addEventListener('mousemove', (e) => {
        if (!isMouseDown) return;

        e.preventDefault();
        const x = e.pageX - lecList.offsetLeft;
        const walk = (x - startX) * 1;
        lecList.scrollLeft = scrollLeft - walk;
    });
})();